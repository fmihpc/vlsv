...
namespace vlsvplugin {
   ...
   bool VisitQuadMultiMeshMetadata::read(VLSVReader* vlsv,const std::map<std::string,std::string>& attribs) {
	 ...
	 return false;
      ...
      // Read XML tag 'MESH_ZONES' to figure out how many mesh blocks we have:
	 ...
	 debug3 << "VLSV\t\t Mesh has " << it->second << " mesh blocks" << endl;
	 numberOfMeshBlocks = atoi(it->second.c_str());
      }
   ...
   bool VisitQuadMultiMeshMetadata::readDomains(VLSVReader* vlsv) {
      // Exit if domain info has already been read:
	 ...
	 return false;
      ...
      if (vlsv->read("MESH_ZONES",attribs,0,numberOfMeshBlocks,domainInfo) == false) {
	 ...
	 return false;
      ...
      domainOffsets   = new uint64_t[numberOfMeshBlocks+1];
      ghostOffsets    = new uint64_t[numberOfMeshBlocks+1];
      variableOffsets = new uint64_t[numberOfMeshBlocks+1];
      domainOffsets[0]   = 0;
      ghostOffsets[0]    = 0;
      variableOffsets[0] = 0;
      for (int64_t i=0; i<numberOfMeshBlocks; ++i) {
	 ...
	 variableOffsets[i+1] = variableOffsets[i] + domainInfo[i*2+0]-domainInfo[i*2+1];	 
      ...
      N_ghostCells = ghostOffsets[numberOfMeshBlocks];
      N_realCells  = variableOffsets[numberOfMeshBlocks];
