...
bool VLSVReader::read(const std::string& tagName,const std::list<std::pair<std::string,std::string> >& attribs,
   ...
   if (VLSVReader::readArray(tagName,attribs,begin,amount,buffer) == false) {
   ...
   for (uint64_t i=0; i<amount; ++i) {
	 ...
	 VLSV::convertValue<T>(outBuffer[i*vectorSize+j],ptr,datatype,dataSize,false);	 
	 ptr += dataSize;
      }
   }
